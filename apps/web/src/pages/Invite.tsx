...
export function InvitePage() {
  ...
  */
  ...
  useEffect(() => {
    ...
    /*
      姿势的切换时点要和 CSS 那张时间表对齐（4.5 秒 = 4500ms）。
      表是：0~28% 进场就位（这一段纸一动不动）→ 34% 抓住 →
      44/54/76% 三次使劲 → 100% 拖出去。
      28% × 4500 = 1260ms，所以「抓住」落在 1300ms 左右。
      之前写的是 150/330ms —— 那时候进场只有 12%，现在是 28%，
      所以整组时点往后挪，否则小人还没站定就已经在「使劲」了。
    */
    const plan: Record<Move, [number, PuppetMood][]> = {
      // 拉（左/右）：进场 → 侧身 → 抬手够过去 → 抓住 → 全身使劲
      pull: [
        [0, 'reach'],
        [900, 'reach'],
        [1300, 'grab'],
      ...
      {showAll ? (
        ...
        <>
          ...
          >
            ...
            {/*
              「压」那一支的回来方式：**从上面掉一根绳子**，把新的这一页拉上来。
              （用户原话：「上面的话就是从上面把线压下去，然后再从上面掉一根绳子把给拉上来」）

              ⚠️ 绳子挂在 .screen 上，**不能**挂在 .screen-inner 里面 ——
                 .screen-inner 有 overflow: hidden（用来裁 emoji 徽章），
                 挂进去之后绳子上面那一截会被裁掉，就只剩纸里面一条线，看不出"从上面垂下来"。
                 挂在这一层、z-index 放在纸下面，才是"一根绳子从屏幕顶垂下来勾住信纸"。
            */}
            {leaving === null && enterFrom === 'push' && (
              <span className="move-rope" aria-hidden="true" />
            )}
          </section>
