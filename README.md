...
| 选时段 | 我的申请 |
| --- | --- |
| ![时段](docs/shots/03-slots.png) | ![我的申请](docs/shots/05-my-applications.png) |
| 工作时间段会被「冻住」，选不了 | 交过什么、批没批，一页看完；找不到时正面说明原因 |

| 驳回结果 | 后台 |
| --- | --- |
| ![驳回](docs/shots/06-rejected.png) | ![后台](docs/shots/04-admin.png) |
| 审批意见原样换行显示；「重新填一份」直接进表单，不用再选一遍身份 | 按约会日期排序、按日期窗口筛选、批量操作、导出 |
