...
- **用户端**：一个链接打开 → 我要约牛马 / 我的记录 两条路
- **申请**：选身份 → 选日期和时段（工作时段会被「冻住」）→ 填地点和留言 → 提交
- **邀请**：牛马在后台定好时间地点 → 生成一条链接 → 好友点开是**一封信**（四套材质 + 拆信动画），
  接受或婉拒，**然后两边能来回留言** —— 商量「几点到」「要不要带伞」才是重点
- **再进来不用重选**：交过申请之后，点链接直接回到你自己的回执 / 驳回结果；想看全部申请去 `/mine`
- **后台**：口令登录 → 审批列表 / 改状态 / 备注 / 删除 / 导出，以及改系统配置（改完立即生效，不用发版）
- **不需要登录**：好友端不登录，用浏览器里的随机编号记住自己的申请（代价见「我的申请」那一页的说明）
...
| 首页 | 选身份 | 选时间 |
| --- | --- | --- |
| ![首页](docs/shots/01-entry-ways.png) | ![选身份](docs/shots/02-choose-identity.png) | ![日历](docs/shots/03-calendar-baby.png) |
| 两条路：我要约牛马 / 我的记录 | 四套身份各有各的一套皮：字体、配色、动效都不同 | 底色只说「有没有空」，休息日和今天用小标记 |

| 选时段 | 我的记录 | 驳回结果 |
| --- | --- | --- |
| ![时段](docs/shots/04-slots.png) | ![我的记录](docs/shots/10-my-records.png) | ![驳回](docs/shots/06-rejected.png) |
| 工作时间段会被「冻住」，选不了 | 交过什么、收到过什么一页看完；找不到时正面说明原因 | 审批意见原样换行；「重新填一份」直接进表单 |

### 收到邀请 ⭐

牛马在后台定好时间地点，生成一条链接发出去。好友点开看到的**不是表单，是一段一页一页推进的分镜**：

> 一封信一次性展开是**观赏**（「哇，好看」）；
> 一页一页推进是**期待**（「然后呢？」）。
> 这一页要的是后者 —— 每屏只讲一件事，屏与屏之间由小人做过渡，节奏交给好友的手指。
>
> 赶时间的人点「看全部」就能一次看完，不把人关在流程里。

| 好宝宝 · 奶油纸 | 好姐妹 · 珠光信笺 | 好兄弟 · 不许拒绝 |
| --- | --- | --- |
| ![邀请·宝宝](docs/shots/07-invite-baby.png) | ![邀请·姐妹](docs/shots/08-invite-sister.png) | ![邀请·不许拒绝](docs/shots/09-invite-nodecline.png) |
| 火漆砸下、蜡向四周溢开 | 「先把日子定下来」+ 烫金大字 | 小牛马坐上了接受按钮（犄角+领巾＝男版） |

| 抓住婉拒按钮 | 揉成一小团纸 |
| --- | --- |
| ![揉团](docs/shots/14-invite-crumple.png) | ![坐下](docs/shots/15-invite-sit.png) |
| 一个小人抓着它、把它揉成一小团纸 | SVG 湍流滤镜做出真实褶皱，不是简单缩小 |

> **邀请页和填写页刻意不一样。** 填写页天天用、要填一堆东西，所以克制；
> 邀请页一辈子看一两次，所以放开了做 —— 四套**材质**（牛皮纸 / 珠光 / 奶油 / 红头文件）、
> 火漆溢蜡、墨迹书写、光尘，以及**一个会抢按钮的小人**。
>
> 小人**分男女**（后台设一次，全局生效）：男的是犄角 + 粗眉 + 领巾，女的是大蝴蝶结 + 长睫毛 + 围裙 ——
> 一眼看得出是两个人。它代表牛马本人，所以不该每个身份一个形象。

### 后台

| 审批列表 | 邀请 | 编辑邀请 |
| --- | --- | --- |
| ![后台](docs/shots/11-admin.png) | ![邀请](docs/shots/12-admin-invites.png) | ![编辑](docs/shots/13-admin-invite-edit.png) |
| 按约会日期排序、按日期窗口筛选、批量操作、导出 | 每条能复制链接、编辑、对话、删除 | 四个身份一键套预设，下面有实时预览 |

### 四套体验：不只是换皮

**换皮 = 字体 + 颜色；换体验 = 几何 + 材质 + 动效节奏 + 交互逻辑 + 反馈方式 + 文案口吻，六样一起换。**

同一颗按钮，做成 120ms 线性和 420ms 回弹，读起来**是两个人**。所以「时长」本身就是性格：

| 好兄弟 · 别磨叽 | 好姐妹 · 要好看 | 好宝宝 · 来嘛不急 | DAD&MUM · 按流程办 |
| --- | --- | --- | --- |
| ![印章·兄弟](docs/shots/19-invite-seal-brother.png) | ![印章·姐妹](docs/shots/19-invite-seal-sister.png) | ![印章·宝宝](docs/shots/19-invite-seal-baby.png) | ![印章·家人](docs/shots/19-invite-seal-dadmam.png) |
| 正方形印章 · 翻页直接换 · **5 步**（不写小作文） | 椭圆印章 · 横滑 · 圆点进度 | **爱心印章** · 弹上来 · 爱心进度 · **撒花回执** | 正圆印章 · 淡入 · 「第 3 / 6 **项**」 · 打勾回执 |

| 四张身份卡，形状就不同 |
| --- |
| ![四张卡](docs/shots/18-entry-card-shapes.png) |
| 直角方形 0px ｜ 大圆角 34px ｜ 爱心圆角 26px + 光晕 ｜ 最普通的标准圆角 8px |

| 选了之后各给各的记号 |
| --- |
| ![宝宝选中](docs/shots/21-select-baby.png) ![家人选中](docs/shots/21-select-dadmam.png) |
| 兄弟**没有动画** ｜ 姐妹柔柔放大 ｜ 宝宝**弹一下 + 冒一颗心** ｜ 家人**一个勾** |

设计纲领与全部实测值见 `docs/EXPERIENCE.md`。

> 上面是「好宝宝」那套皮。另外三套（好兄弟 / 好姐妹 / DAD&MUM）风格完全不同 ——
> 字体、配色、动效各一套，连「中午太热」「晚上太冷」的抱怨口吻都不一样。
