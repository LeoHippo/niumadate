...
它决定了后面所有事 —— 因为**中国大陆的服务器对外做网站必须备案**。

| | 中国大陆 | 香港 / 新加坡 |
| --- | --- | --- |
| 2 核 2G 轻量 | **38 元/年**（新用户活动价） | **336 元/年**（28 元/月） |
| 备案 | **必须** | **不用** |
| 从买到能访问 | **再等 1~3 周**（备案） | 买完当天就能用 |
| 从大陆访问 | 最快 | 稍慢几十毫秒，**基本感觉不到** |

价格差 9 倍，但**国内那 38 元买不到时间**：

> **备案是什么。** 就是把你的域名和服务器实名登记到工信部，
> 需要：一个**已实名**的域名 + 一台**已买满 3 个月**的服务器 + 你的身份证。
> 流程是「提交 → 云厂商初审（1~2 天）→ 管局审核（7~20 天，各省不同）」。
> **在备案通过之前，80 / 443 端口不能对外提供服务** ——
> 也就是说服务器开机了、代码部署好了，但好友点链接还是打不开。
>
> 所以：**想当天就能发给好友，就别选国内。**
...
| 你的情况 | 选这个 | 一年花多少 |
| --- | --- | --- |
| **想马上就能发给好友** | 阿里云 / 腾讯云 **香港轻量** 2 核 2G | **336 元**（先买 1 个月 28 元试试也行） |
| 不着急，想长期最省 | 国内轻量 38 元/年 + 备案 | **约 70 元**（38 + 域名） |
| 完全不想花钱 | 甲骨文云「永久免费」 | **0 元**，但注册要信用卡验证、容易失败、有被回收的风险 |
| 电脑能一直开着 | Cloudflare Tunnel 从家里电脑穿透 | **0 元**，但家里电脑不能关机，且国内访问 Cloudflare 不稳 |

**这个应用要的配置极低**：一个 Node 进程 + 一个 SQLite 文件，
**1 核 1G 都够**，2 核 2G 已经很宽裕。别被「4 核 8G 更专业」忽悠。

### 另外两件事

| 问题 | 建议 |
| --- | --- |
| **要不要域名** 🚩 | **要**。用 IP 就没法上 HTTPS，微信里分享 `http://1.2.3.4:8787` 这种链接体验很差，而且微信对裸 IP 有时会拦。一个 `.top` / `.xyz` 域名首年十几块。 |
| **代码怎么传** | Git（推荐）/ 直接打包上传，见第 4 步 |

> **为什么不用 Vercel / Railway 那类平台？**
> 这个应用把数据存在一个 SQLite 文件里，需要**持久磁盘** —— 那类平台的临时文件系统重启就没了。
> 而且好友主要在微信里点，国内 / 香港的访问体验明显更好。
...
2. 配置：**2 核 2G**，系统选 **Ubuntu 24.04**（本文档按 Ubuntu 写）
3. **地域**：香港 / 新加坡（不想备案就选这里）
4. 买完记下：**公网 IP**、**root 密码**（或密钥）

**防火墙**：在控制台的「防火墙 / 安全组」里放行 **80** 和 **443** 端口。（22 一般默认开着。）

> ⚠️ **只放行 80 / 443，不要放行 8787。**
> 后端只监听 `127.0.0.1`，外面由 Caddy / Nginx 转发进来。
> 如果把 8787 直接开到公网，就等于把后台暴露在一个没有 HTTPS 的端口上。
